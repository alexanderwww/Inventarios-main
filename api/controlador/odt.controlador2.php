...
class ControladorOdt extends apiModel
{
    ...
    function dataOdtTabla($tabla)
    {
        ...
        if (!empty($tabla)) {
            ...
            $query='SELECT odt.*, pt.Nombre AS nameProducto, us.User AS nameUser, pt.SubNombre AS SubNombre FROM odt 
        ...
        if (!empty($tabla)) {
            ...
            inv.PrecioLitros, pro.Formulacion, inv.Total AS InventarioActual , pf.`Version`
                        FROM $tabla pro 
                            ...
                            LEFT JOIN productoformulado pf ON pf.IdContenido = pro.Id
    ...
    function createOdt($dataOdt, $tabla, $productos, $user)
    {
        ...
        if (is_array($dataOdt) && isset($tabla)) {
            ...
            if ($status == 1) { //Cuando el estatus viene en 1 significa que se creara una nueva version
                foreach ($productos as $valueItemV) {
                    $idProV=$valueItemV['IdProducto'];
                    $sqlProV = 'SELECT tipoUnidad, Nombre FROM productos WHERE Id=' . $idProV;
                    $respProV = $this->getAllTable($sqlProV);
                    $tipoUnidad = $respProV->fetch_assoc();
                    if($tipoUnidad['tipoUnidad']!='2'){
                        $msj="Error en la formulacion del producto ".$tipoUnidad['Nombre']." solo se aceptan productos con tipo de unidad Litros";
                        $mensaje = array("success" => $success, "data" => $data, "message" => $msj);
                ...
                $Color = $newPro['Color'];
                $Hazmat = $newPro['Hazmat'];
                $Marca = $newPro['Marca'];
                $Concentracion = $newPro['Concentracion'];
                $Uso = $newPro['Uso'];
                // $subName = $newPro['subNombre']?$newPro['subNombre']:'';
                $newName =$dataOdt['NombreProducto'];
                $Formulacion = 1;
                $queryInsert = 'INSERT INTO productos (Nombre, Densidad, Color, Hazmat, Marca, Concentracion, Uso, Formulacion) 
                VALUES ("' . $newName . '",' . $Densidad . ',"' . $Color . '",' . $Hazmat . ',"' . $Marca . '",' . $Concentracion . ',"' . $Uso . '",' . $Formulacion . ')';
                // print_r($queryInsert);
