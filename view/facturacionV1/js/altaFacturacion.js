$(document).ready(function () { //Informacion al cargar la pagina
    ...
    // NOTA CAMBIOS ALEXANDER CAMBIO: SE METIO LOS DATOS EN UNA FUNCION ASINCRONA 
    const initRow=async()=>{
        ...
        // NOTA CAMBIO ALEXANDER SE AGREGO VALIDACION 
        if($('#idPedido').val()){
...
$('#tablaPrincipal tbody').on('click', '.btnDeleteRowConceptos', async function () {
    ...
    // CAMBIO ALEXANDER 20-04 SE PUSO ASYNC

$('#container_CFDI').on('click', '.btnDeleteRowCFDI',async function () {

    // let tableActive = document.querySelector(`#container_CFDI`);

    // if (tableActive.childElementCount != 1) {

    // CAMBIO ALEXANDER 20-04 SE AGREGO VALIDACIONES DE EDIT

    if($(this).attr("idrelacional")){


        let idRelacional=$(this).attr("idrelacional");

        console.log("Es edit "+idRelacional)
        
        let accion={Accion: 'factura', Tabla: 'factura', 'id': idRelacional,"Select":'deleteCFDIrelacionados'};

        await postData(rutaApi,accion).then(async(response)=>{
...
// Falta keys inputs 
// CAMBIOS ALEXANDER 04-19 ; SE AGREGO NUEVO CLASE PARA ATTR DISABLED

const cloneRowConceptos = (key) => (
    `
    <tr>
    <th>
        <button class="btn btn-danger btnDeleteRowConceptos statusEditTable btnDeleteRowConceptos_${key}" key="${key}">X</button>
    </th>
    <th>
        <input  onkeyup="statusValidationInput(this);"  class="form-control statusEditTable validateItemsConceptos" type="text" id="itemOdv_${key}" key="${key}">
    </th>
    <th>
        <input  onkeyup="statusValidationInput(this);"  class="form-control statusEditTable validateItemsConceptos" type="text" id="itemCodigo_${key}" key="${key}">
    </th>
    <th>
        <input  onkeyup="statusValidationInput(this);"  class="form-control statusEditTable validateItemsConceptos" type="text" id="itemClaveProdServ_${key}" key="${key}">
    </th>
    <th>
        <input  onkeyup="statusValidationInput(this);"  class="form-control statusEditTable validateItemsConceptos" type="text" id="itemDescripcion_${key}" key="${key}">
    </th>
    <th>
        <input  onkeyup="statusValidationInput(this);" oninput="statusBalance(this);" class="form-control statusEditTable validateItemsConceptos mask-pesos" type="text" id="itemCantidad_${key}" key="${key}">
    </th>
    <th>
    ...
    <th>
        <div class="input-group">
            ...
            <input onkeyup="statusValidationInput(this);" oninput="statusBalance(this);" class="form-control statusEditTable validateItemsConceptos mask-pesos" type="text" id="itemPrecio_${key}" key="${key}">
        </div>
    </th>
    ...
    <th>
        <div class="input-group">
            <span class="input-group-text">$</span>
            <input class="form-control mask-pesos statusEditTable" type="text" id="itemTotal_${key}" key="${key}" disabled>
        </div>
    </th>
...
$('#insertFactura').on('click',async()=>{
  ...
  let dataBalanceConceptos = clearDataConceptos(
    await getDataForms("getDataBalanceConceptos")
  );
...
const getPeditoFactura=async(Id)=>{
    ...
    await postData(rutaApi,accion).then(async(arrayData)=>{
        ...
        // NOTA CAMBIOS ALEXANDER : SE COMENTA LA FUNCION alerta EN GETDATA 
            ...
            //         formaPago:1,
            //     },
            //     pedidos:[itemPedido,itemPedido,itemPedido,itemPedido,itemPedido],

            //     totales:{
            //         observaciones:'Description',
            //         subtotal:100,
            //         impuestoRetenido:2,
            //         impuestoTrasladado:1.3,
            //         total:1000
            //     },

            //     cfdis:[itemCFDI,itemCFDI,itemCFDI]
...
const setPedidoCliente = async (arrayData) => {
    ...
    for (item of arrayData) {
        ...
        // $('#itemUnidad_'+keyRow).val(item['unidad']);
...
const setCFDICliente=async(arrayData)=>{
...
// CAMBIOS ALEXANDER 04-19 ; nuevas Funciones
...
for(CFDI of arrayData){
    ...
    // CAMBIOS 20-04 
    $('#btnDeleteRowCFDI_'+keyCFDI).addClass("btnDeleteRowCFDI_edit");
    $('#btnDeleteRowCFDI_'+keyCFDI).attr("idRelacional",CFDI['idRelacional']);
...
const disabledDataFactura=(statusEdit=true)=>{
...
// NOTA CONTINAR PASANDO LOS CAMBIOS EN: ELIMINAR CDFIS YA CARGADOS EN EDIT 
// CONTINUAR EN EDIT YA ENVIA LOS DATOS SOLO QUE NO SE ELEIMINAR
