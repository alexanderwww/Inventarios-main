...
const initFetchs = async () => {
     arrayDataProductos = await methodGetData('?Accion=productos&Tabla=productos&Select=cotizador');
     await newRowTable();
     await newRowTable();
...
const createRowTable = async(key) => {
    ...
    <td>
        <div style="width: 100%;">
            <select onchange="statusItemProducto(this);calculateRow(${key})" class="padding_top_bottom_0 form-control form-select" id='producto_${key}' key="${key}">
    ...
    <td>
        <div class="col-form-label col-md-6 col-sm-6 padding_top_bottom_0" style="display: flex; margin-left: -1px; border-top-left-radius: 0;border-bottom-left-radius: 0;margin:auto;">
            <input onkeyup="calculateRow(${key})" onchange="calculateRow(${key})" class="form-control padding_top_bottom_0 borderRadiusInputPercentaje stylePorcenta_input" type="text" id="porcentaje_${key}" key="${key}">
            <span class="input-group-text padding_top_bottom_0 stylePorcenta_" style=" margin-left: -1px; border-top-left-radius: 0;border-bottom-left-radius: 0;">%</span>
        </div>
    </td>
...
const calculateMargen=()=>{

    let margen = $('#margen_porcentaje').val();

    let costoPorLitro = maskMoney($('#balance_costoPorLitro').text());
    let margenNeto=(margen / 100)+1;
    ...
    let cantidadAFabricar = clearNumber( $('#cantidadFabricar').val() );

    $("#margen_precioTotalVenta").text(maskMoney(cantidadAFabricar * precioVentaPorlitros));
};
