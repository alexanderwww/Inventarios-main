...
// en modulo "odt" en "odt.js" en la funcion "tablaPrincipal()"
    ...
    var tablaSSP = $('#tablaPrincipal').DataTable({
        ...
        createdRow: function (row, data) {
            ...
            if(data.Status==2){
                ...
                // VERDE
                $(row).css('background-color',"rgb(144, 238, 144)");
            ...
            if(data.Status==0){
                // ROJO
                $(row).css('background-color',"rgb(255, 145, 145)");
            }
...
$(document).on('click', '.btnAceptarCerrar', function (e) {
    ...
    cerrarOdt(idModal,status);



});

const cerrarOdt = async (id, status) => {

    let accion = { "Accion": "odt", 'Select': 'cerrar','Status': status, 'Id': id };
