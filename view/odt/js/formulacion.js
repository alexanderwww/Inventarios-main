...
$('.btnAceptarAlta').on('click',async()=>{
        ...
        if(respValidar('validarDataAlta')){
            ...
            if(!soloNumeros($('#CantidadFabricar_alta').val())){
                ...
                showAlert("Alerta",'La cantidad a fabricar deber solo números',"info");
...
const initTableVersion=async(countTable)=>{
        ...
        // CAMBIO ALEXANDER 20-04 
        boxTemplateTable.querySelector('.titleVersionItem_Example').id='titleVersionItem_'+countTable;
        boxTemplateTable.querySelector('.titleVersionItem_Example').classList.remove('titleVersionItem_Example')
        // -----------------------END 
...
const initVersionesProducto=async(IdProducto)=>{
    ...
    for(version of data['items']){
        ...
        var numVersion = version['Version'];
    }

    // console.log(data.data.Nombre);

    numVersion=parseInt(numVersion)+1;
    ...
    for(version of arrayVersion){
            ...
            document.getElementById(`NombreItem_${version['Version']}`).value=data.data.Nombre+"_"+numVersion;

            // CAMBIO ALEXANDER 20-04 
            console.log(version);
            document.getElementById(`titleVersionItem_${version['Version']}`).textContent=version.Nombre;
...
$('.containerPrincipalTabs').on('click', '.checkHabilitarEdit', function (element) {
    ...
    if(this.checked){
        ...
        // let nombreTemp=document.querySelector('#NombreItem_'+numberContainer).value;
        
        // NOTA CAMBIO 999 COMENTADO

        // document.querySelector('#NombreItem_'+numberContainer).value='';
        // $('#NombreItem_'+numberContainer).attr('disabled', true);
        
        $('#NombreItem_'+numberContainer).parent().parent().css('display','block');
        // --------------------------
        ...
        // NOTA CAMBIO 999 COMENTADO
        $('#NombreItem_'+numberContainer).parent().parent().css('display','none');
...
const agregarFileTable=async(fila,numContainer)=>{
    // NOTA CAMBIO222 SPRINT: SE AGREGA NUEVAS CLASES _totalProducto
    // Y OTRAS MAS A LOS CONTAINER DE DATOS 
    ...
    newFila=`
        ...
        <td class='totalProducto totalProducto${fila['Id']}' >${fila['Total']}</td>
...
const agregarFilaTablaNueva=async()=>{
    // NOTA CAMBIO SPRINT: SE AGREGA NUEVAS CLASES totalProducto
...
// {/* <tr class="nuevoItem"> */}
    ...
    <td class='totalProducto totalProductoItemsNuevo${contadorFilaposition}' >
...
const getTotales=async(key)=>{

    // porcentajeItems 
    // litrosBarrilItems 
    // litrosProducionItems 
    // costoLitroItems 
    // importeItems 

    let tableActive=document.querySelectorAll(`#tablaTabs_${key} tbody tr`);

    let arrayData=[];

    tableActive.forEach(column=>{

        arrayData.push({
            totalProducto:column.querySelector('.totalProducto').textContent,
            porcentaje:column.querySelector('.porcentajeItems').value,
            barrilesProduccion:column.querySelector('.litrosBarrilItems').value,
            litrosProduccion:column.querySelector('.litrosProducionItems').value,
            costoPorLitro:column.querySelector('.costoLitroItems').value,
            importeTotal:column.querySelector('.importeItems').value
        })
    })
...
const clearTotales=async(arrayData)=>{


    arrayData=arrayData.map((item)=>{

        return{
            totalProducto:clearText(item.totalProducto),
            porcentaje:clearText(item.porcentaje),
            barrilesProduccion:clearText(item.barrilesProduccion),
            litrosProduccion:clearText(item.litrosProduccion),
            costoPorLitro:clearText(item.costoPorLitro),
            importeTotal:clearText(item.importeTotal)
        }
...
const totalesData=async(arrayData)=>{
    
    // let arrayData=await clearTotales();

    let totalProducto=0;
    let porcentaje=0;
    let barrilesProduccion=0;
    let litrosProduccion=0;
    let costoPorLitro=0;
    let importeTotal=0;
    ...
    arrayData.forEach(item => {
        ...
        barrilesProduccion+=item.barrilesProduccion
        litrosProduccion+=item.litrosProduccion
        costoPorLitro+=item.costoPorLitro
        importeTotal+=item.importeTotal

    })

    let data ={
        totalProducto:maskMoney(totalProducto),
        porcentaje:maskMoney(porcentaje),
        barrilesProduccion:maskMoney(barrilesProduccion),
        litrosProduccion:maskMoney(litrosProduccion),
        costoPorLitro:maskMoney(costoPorLitro),
        importeTotal:maskMoney(importeTotal)
    }

    return data
}

const writeTotales=async(key=false)=>{

    if(key===false){
        
        key=statusTableActive();

    }
    console.log('LLamada')

    let arrayData=await getTotales(key);

    arrayData=await clearTotales(arrayData);

    let totales=await totalesData(arrayData);

    $('.totales_producto').text(totales.totalProducto);
    $('.totales_porcentaje').val(totales.porcentaje);
    $('.totales_litrosBarril').val(totales.barrilesProduccion);
    $('.totales_litrosProducion').val(totales.litrosProduccion);
    $('.totales_costoLitro').val(totales.costoPorLitro);
    $('.totales_importe').val(totales.importeTotal);
}

const clearText=(string)=>{

    let stringSinComas = string.replace(/,/g, '');

    if(string==""){
        return 0;
    }

    if(!isNaN(stringSinComas)){
        
        return parseFloat(stringSinComas);
    }
    return 0;
}
