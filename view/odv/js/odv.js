    ...
    var tablaSSP = $('#tablaOdv').DataTable({
        ...
        createdRow: function (row, data) {
            ...
            if(data.Status==="Cancelado"){
                $(row).css('background','rgb(255, 145, 145)')
                // rgb(255, 145, 145)
            ...
            if(data.Status==="Cerrado"){
                ...
                $(row).css('background','rgb(144, 238, 144)');
...
// MODIFICACION ANGEL 20/04/23
    let accion = { "Accion": "odv", "Tabla": "odv", 'Select': 'cancelar','Status': status, 'Id': id };
...
$(document).on('click', '.btnCerrarTabla', function (e) {

    let id = e.target.id;

    id = id.substring(2);

    let btnModalCerrar=document.querySelector('.btnAceptarCerrar');

    btnModalCerrar.removeAttribute('id');





    btnModalCerrar.setAttribute("id",id);



    $('#labelOdv').text('¿Desea Cerrar la orden de venta #' + id + '?');
...
$(document).on('click', '.btnAceptarCerrar', function (e) {
    ...
    cerrarOdv(idModal,status);



});

const cerrarOdv = async (id, status) => {

    let accion = { "Accion": "odv", 'Select': 'cerrar','Status': status, 'Id': id };
    ...
    return await fetch(rutaApi, {
        ...
        .then(respuesta => {
            ...
            if (respuesta['success']) {
                ...
                let comentario = "Cerro la Orden de compra con el ID:"+id;
                setBitacora('8', comentario, modulo);
                showAlert("Correcto", respuesta['messenge'], "success")
...
const insertValueFormEdit=(data)=>{
    ...
    document.getElementById('SubTotal_edit').value=mascarMonedaInputs(data['Subtotal']);

    document.getElementById('Iva_edit').value=mascarMonedaInputs(data['Iva']);

    document.getElementById('Total_edit').value=mascarMonedaInputs(data['Total']);
...
const insertDataItem = async (key, itemData) => {
    ...
    document.getElementById('Cantidad_edit' + key).value = mascarMonedaInputs(itemData['Cantidad']);

    document.getElementById('Precio_edit' + key).value = mascarMonedaInputs(itemData['Precio_Litro']);
    ...
    document.getElementById('Total_edit' + key).value = mascarMonedaInputs(itemData['Total']);
...
async function postData2(accion) {

    return await fetch('../pdf/orderOfOrders.php', {
        method: 'POST',
        body: JSON.stringify(accion),
        headers: { 'Content-Type': 'application/json' }
...
// // función para recibir los datos y mostrar el PDF en pantalla
// function showPDF(data) {
//     console.log(data);
//     // crea un objeto de tipo Blob a partir de los datos recibidos
//     var blob = new Blob([data], { type: 'application/pdf' });

//     // crea un objeto de tipo URL a partir del Blob
//     var url = window.URL.createObjectURL(blob);

//     // crea un objeto de tipo <embed> para mostrar el PDF en pantalla
...
function showPDF(data) {
  ...
  function descargarPDF(data) {
    ...
    // crea un elemento <a> para descargar el PDF
    var link = document.createElement('a');
    link.href = url;
    link.download = 'archivo.pdf';
  
    // agrega el elemento <a> al DOM y haz clic en él para descargar el PDF
...
const postData=async(accion)=>{
      // llamada a la función pdf() y manejo de la respuesta
  await fetch('../pdf/factura2.php', {
    method: 'POST',
    body: JSON.stringify(accion), // aquí debes incluir los datos que quieres enviar a la función pdf()
  })
  ...
  .then(buffer => {
    // return;
    // convierte el buffer en un Uint8Array para pasarlo a la función showPDF()
...
const createPDF=async(id)=>{
    ...
    // postData({'Accion':'test',"data":dataPDF['data']});
...
const downloadPDF= (dataPDF) => {
    ...
    let urlPDF ="../pdf/orderOfOrders.php"; 
...
$('body').on('click','.btnEvidencias',function (e){
...
const getEvidencias=async(id)=>{

    let accion = '?Accion=odv&countEvidencias=1&Tabla=archivospedidos&IdPedido='+id;
    ...
    .then(async(respuesta)=>{
        ...
        if(typeof respuesta['data']!='string'){
            ...
            document.getElementById('containerEvidenciasUserEdit').innerHTML=`<p class="text-center" style="width:100%;">No hay documentos.</p>`;
...
const insertDataEvindencias = async (id, evidencia) => {
    ...
    let rutaArchivo = "../../Data/EvidenciasPedidos/";

    let rutaVista = "../../Data/EvidenciasPedidos/";
    ...
    let urlCopy = evidencia['url'] + `Data/EvidenciasPedidos/${evidencia['NombreArchivo']}`;
...
const getStatusEvidencias=async(id,countFile)=>{
    ...
    let accion=`?Accion=odv&statusEvidencia=1&Tabla=archivospedidos&IdPedido=${id}&CantidadPedido=${countFile}`;
...
$('body').on('click','.btnCopyArchivo',function(){    
...
$('body').on('click', '.file_download', function() {
    ...
    linkSitio = "../../Data/EvidenciasPedidos/" + nameFile;
...
$('body').on('click','.btnCerrar',function(){
    ...
    $('#modalCerrar').modal('show');
    idPedidoCerrar=$(this).attr('name');


    $("#nameCerrar").text('#'+idPedidoCerrar);
    return;
})

$('#btnAceptarCerrar').on('click',()=>{

    cerrarPedido(idPedidoCerrar,2);

    return;
})

const cerrarPedido = async (id, status) => {
    ...
    return await fetch(rutaApi, {
        ...
        .then(respuesta => {
            ...
            if (respuesta['success']) {
                ...
                let comentario = "Cerro el pedido con el ID:"+id;
...
$('#Cliente_alta').on('change',async function(){

    let id = this.value;
    console.log(this.value);
    let accion='?Accion=odv&proceso=verificarSaldo&Id=' + id;
    let respuesta = await verificarSaldo(accion);
    console.log(respuesta);
    if(respuesta["success"]===false){
    showAlert("Alerta", respuesta['messenge'], "info")
    $("#Cliente_alta").css({ 'border-color': 'rgba(116, 0, 0, 0.6)', "border-weight": "3px" });
    $("#ul_Cliente_alta").css({ 'display': 'block' });
    $("#ul_Cliente_alta").text(respuesta['messenge']);

    }

})
const verificarSaldo = async (accion) => {
