...
$('.btnModalAlta').on('click',async()=>{
    ...
    // $('#tipoUnidad_alta').trigger("chosen:updated");
    
    await getSelect(accion,'tipoUnidad_alta')
    // $('#tipoUnidad_alta').prop('disabled', false).trigger("chosen:updated");
...
const statusViewForm=(status)=>{
    ...
    if(status){
        ...
        document.getElementById('SubNombre').removeAttribute('disabled');
        ...
        document.getElementById('SubNombre').setAttribute('disabled',true);
...
const insertDataItem=async(key,itemData)=>{
    ...
    document.getElementById('ProductoPrimario_edit'+key).value=itemData['IdContenido'];
