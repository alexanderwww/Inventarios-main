...
$(document).on('click', '.btnModalCancelar', function (e) {
    ...
    let status = 0;
    cancelarCompra(idUser, status);


});
const cancelarCompra = async (id, status) => {

    let accion = { "Accion": "compras", "Tabla": "compras", 'Status': status, 'Id': id };
...
const insertDataInputsFormView=(data)=>{
    ...
    document.getElementById('Entrada_View').value=data['Entrada'];
    document.getElementById('Proveedor_View').value=data['NombreProveedor'];
    document.getElementById('Precio_View').value=data['PrecioLitro'];
